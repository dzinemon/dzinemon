...
document.addEventListener('DOMContentLoaded', () => {
  ...
  var generateTableItem = function (item) {
    return `
      <tr>
        <td class="td-c country he-opac hoverable he-hl he-tu" style="min-width: 160px;" data-country="${item.country}">${item.country}</td>
  ...
  window.onload = function () {
    ...
    // document.getElementById('total_cases').innerText = totalCasesGlobal;
    // document.getElementById('total_death').innerText = totalDeathGlobal;

    // document.getElementById('percent_death').innerText = `(${deathPercentage}%)`;

    // document.getElementById('country_tabs').innerHTML = generateTabs(allData);
    ...
    getAllItemsForConfirmed.forEach(i => {
      i.innerHTML = totalCasesGlobal;
    });

    getAllItemsForDeath.forEach(i => {
      i.innerHTML = totalDeathGlobal;
    });
    getAllItemsForRecovered.forEach(i => {
      i.innerHTML = totalRecoveredGlobal;
    });
